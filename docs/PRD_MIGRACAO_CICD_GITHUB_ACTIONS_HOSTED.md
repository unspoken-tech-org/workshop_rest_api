...
| todos | — | **v1.21:** `deploy.sh` extraído de heredoc inline para `.github/scripts/deploy.sh` — elimina `sed -i` strip de indentação e heredocs frágeis no YAML; `.env` reescrito com `echo` statements (sem heredoc); script copiado via `scp` para o servidor |
| todos | — | **v1.22:** `verify.sh` e `rollback.sh` extraídos de SSH one-liners (~800 chars) para `.github/scripts/` — scripts externos eliminam quoting complexo, condicionais inline e command substitutions ilegíveis; copiados via `scp` e executados via `ssh` |
...
| 1.33 | 23/06/2026 | Eng. | **`docker/login-action` adicionado ao job `deploy` — corrige `config.json` strategy.** (1) **Problema:** a estratégia v1.24 copiava `~/.docker/config.json` do runner do `deploy` para o servidor, mas o runner do `deploy` não tem `docker/login-action` — o login só estava no job `build` (outro runner). O `config.json` copiado no servidor tinha credenciais inúteis, e o `docker pull` falhava com `unauthorized`. (2) **Solução:** adicionar step `docker/login-action@<SHA>` no job `deploy` (antes do `Deploy via SSH`) com `registry: ghcr.io`, `username: ${{ github.actor }}`, `password: ${{ secrets.GITHUB_TOKEN }}`. (3) **`deploy.yml`** — 1 step adicionado no job `deploy`. (4) **`deploy.sh`** — sem mudança (já usava `mv config.json.tmp`). (5) **Resultado:** `config.json` no runner do `deploy` tem `GITHUB_TOKEN` válido; scp copia credenciais funcionais; `docker pull` no servidor autentica corretamente. (6) **Nota:** o job `deploy` tem `permissions: packages: read` — suficiente para login GHCR com escopo de pull. |
| 1.34 | 23/06/2026 | Eng. | **`setup-tailscale` composite removido — `tailscale/github-action` externo em todos os jobs.** (1) **Problema:** o composite `./.github/actions/setup-tailscale` exige `actions/checkout` prévio (composite local, não action externa). Os jobs `verify` e `cleanup` não têm checkout, então o step "Setup Tailscale" quebrava com `Can't find action.yml`. (2) **Alternativa descartada:** adicionar checkout em `verify` e `cleanup` (Opção A) — mesmo padrão mas adiciona 2 steps desnecessários. (3) **Solução:** reverter 3 jobs (`deploy`, `verify`, `cleanup`) para `tailscale/github-action@<SHA>` externa com `tags: tag:ci`; deletar `.github/actions/setup-tailscale/action.yml`. (4) **`deploy.yml`** — 3 substituições diretas. (5) **Resultado:** sem checkout adicional; código mais simples; consistente com o que o `v1.3.0-rc-test` (commit 88f9563) usava. |
