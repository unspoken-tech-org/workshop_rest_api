...
| **LAN (Offline)**  | Split-Brain DNS   | Caddy (Local Proxy) | High availability even during ISP outages    |
...
- **Caddy Proxy:** Handles automatic SSL certificates (via DNS-01 challenge) so the same domain works everywhere with
...
- **QA:** Deploys from feature branches to `api-qa.eletroluk.com` for infrastructure testing (isolated DB and Keys).
