...
public interface ApiKeyController {
    ...
    @Operation(
            summary = "Reset API Key device",
            description = "Clears the device binding and revokes refresh tokens for the API Key.")
    @ApiGlobalResponses
    @ApiResponse(responseCode = "204", description = "API Key device reset successfully")
    @PostMapping("/{id}/reset-device")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @PreAuthorize("hasRole('ADMIN')")
    void resetDevice(@Parameter(description = "API Key identifier") @PathVariable Long id);
